...
def loss_mse(w1, w2, s2, yb, rf):
    mse_i=np.sum((s2-yb)**2, axis=1)
    mse=np.mean(mse_i)
    return mse+0.5*rf*((w1**2).sum()+(w2**2).sum())

def grad_mse(w1, w2, batch_size, xb, yb, y1, s1p, s2, rf):
...
def loss_softmax(w1, w2, s2, yb, rf):
    maxval=-np.amax(s2, axis=1)
    s2_exp=np.exp(s2+maxval[:,np.newaxis])
    s2_exp_y=s2_exp[np.arange(batch_size), np.argmax(yb, axis=1)]
    sum_s2_exp=np.sum(s2_exp, axis=1)
    cce=-np.log(s2_exp_y/sum_s2_exp[:,np.newaxis])
    return cce.mean()+0.5*rf*((w1**2).sum()+(w2**2).sum()), s2_exp, sum_s2_exp

def grad_softmax(w1, w2, batch_size, xb, yb, y1, s1p, s2_exp, sum_s2_exp, rf):
    grad=s2_exp/sum_s2_exp[:,np.newaxis]
...
def fit(x, y, x_test=None, y_test=None, n_neuronas=100, lr=1e-3, rf=1e-5, epochs=1, batch_size=100, metric='mse', seed=None, verbose=False):
    ...
    for e in range(epochs):
        ...
        for it in range(niters):
            ...
            if metric=='mse':
                log_loss+=loss_mse(w1, w2, s2, yb, rf)
                dw1, dw2=grad_mse(w1, w2, batch_size, xb, yb, y1, s1p, s2, rf)
            elif metric=='softmax':
                loss_aux, s2_exp, sum_s2_exp=loss_softmax(w1, w2, s2, yb, rf)
                log_loss+=loss_aux
                dw1, dw2=grad_softmax(w1, w2, batch_size, xb, yb, y1, s1p, s2_exp, sum_s2_exp, rf)
            else:
